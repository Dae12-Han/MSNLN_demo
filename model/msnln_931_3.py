...
class MultisourceProjection(nn.Module):
    def __init__(self, in_channel,kernel_size = 3,scale=2, conv=common.default_conv):
        ...
        self.up_attention1 = MultiScaleAttention(ksize=9) 
        self.up_attention2 = MultiScaleAttention(ksize=3)
        self.down_attention = MultiScaleAttention()
        self.upsample = nn.Sequential(*[nn.ConvTranspose2d(in_channel,in_channel,deconv_ksize,stride=stride,padding=padding),nn.PReLU()])
        self.encoder1 = common.ResBlock(conv, in_channel, kernel_size, act=nn.PReLU(), res_scale=1)
        self.encoder2 = common.ResBlock(conv, in_channel, kernel_size, act=nn.PReLU(), res_scale=1)
        self.encoder3 = common.ResBlock(conv, in_channel, kernel_size, act=nn.PReLU(), res_scale=1)
    
    def forward(self,x):
        down_map = self.upsample(self.down_attention(x))
        up_map1 = self.upsample(self.up_attention1(x))
        up_map2 = self.upsample(self.up_attention2(x))

        err1 = self.encoder1(up_map1-down_map)
        final_map1 = down_map + err1

        err2 = self.encoder2(up_map2-down_map)
        final_map2 = down_map + err2 
        
        err = self.encoder3(final_map2-final_map1)
        final_map = final_map2+err
...
class MSNLN_931_3(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        super(MSNLN_931_3, self).__init__()
